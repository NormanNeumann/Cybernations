	...
	private void BuildHexCluster(Node2D parent)
{
	...
	foreach (var tile in _hexTiles)
	{
		...
		var baseKind = tile.Base == HexBase.Wilds
			? StackView.StackBaseKind.Wilds
			: StackView.StackBaseKind.Wasted;

		var overlayKind = tile.Overlay switch
		{
		...
		stackView.Configure(baseKind, overlayKind, false);
		
		stackView.SetMeta("base_type", tile.Base.ToString());
		stackView.SetMeta("overlay_type", tile.Overlay.ToString());
